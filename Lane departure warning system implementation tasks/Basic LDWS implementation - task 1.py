...
def drawLane(linesLeft, linesRight, frameToDraw):
    ...
    if linesLeft and linesRight:
        ...
        if linesLeft[0][2] == 0 and linesRight[0][2] == 0:
            contours = np.array(
                [[x1_1, ymin + RoIymin], [x2_1, ymin + RoIymin], [x2_2, ymax + RoIymin], [x1_2, ymax + RoIymin]])
    ...
    if linesLeft:
        if linesLeft[0][2] == 1:
            cv2.putText(frameToDraw, "Upozorenje!", (int(width / 2) - 140, 150), cv2.FONT_HERSHEY_SIMPLEX, 2,
